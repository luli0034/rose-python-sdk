...
def build_schema_from_sample(
    ...
    for field_name in all_fields:
        ...
        if non_none_values:
            ...
            sample_value = _get_most_common_type(
                {type(v) for v in non_none_values},
                non_none_values
            )
        else:
            sample_value = None
        ...
        schema[field_name] = create_field_definition(
            field_name, sample_value, is_required, is_identifier
        )
...
def validate_and_align_records(
    ...
    for i, record in enumerate(records):
        try:
            aligned_record, warnings = _align_record_to_schema(
                record, schema_fields, strict_validation
            )
