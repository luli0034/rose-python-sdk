...
from ..exceptions import RoseConflictError, RoseAPIError
...
def create_account_with_conflict_handling(
    ...
    account_id: str,
    max_retries: int = 3,
    auto_generate_unique: bool = True
) -> Role:
    """
...
def create_account_and_token_with_conflict_handling(
    ...
    role = create_account_with_conflict_handling(
        client, account_id, max_retries, auto_generate_unique
    )
    
    # Issue token
    role_with_token = client.roles.issue_token(
        role_id=role.role_id,
        tenant_id=role.account_id,
